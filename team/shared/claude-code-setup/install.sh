...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      ...
      echo "  bash install.sh --verify --community  # 両方実行"
      exit 0
      ;;
...
run_verify() {
  ...
  if [ "$skill_count" -ge 27 ]; then
    ok "Skills: ${skill_count}個 インストール済み (24共有 + 3メタ = 27以上)"
  else
    warn "Skills: ${skill_count}個 (期待: 27個以上)"
    issues=$((issues + 1))
  fi
...
# ============================================================
# フラグのみ（--verify / --community）の場合 → 該当処理だけ実行して終了
# ============================================================
if [[ "$FLAG_VERIFY" == true ]] || [[ "$FLAG_COMMUNITY" == true ]]; then
  # フラグ指定時はメインインストールをスキップ
